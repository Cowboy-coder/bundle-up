...
describe 'View Helper', ->
  ...
  describe 'renderJs', ->
    it 'should render the global.js bundle', ->
      request.get 'http://localhost:1338/globalJs', (err, res) =>
        expect(@bundle.js.files[0].namespace).toEqual('global')
        expect(res.body).toContain(@bundle.js.files[0].url)

        expect(res.body).toNotContain(@bundle.js.files[1].url)
        jasmine.asyncSpecDone()
      jasmine.asyncSpecWait()

    it 'should render the global.js and the custom_namespace.js bundle', ->
      request.get 'http://localhost:1338/globalJs/custom_namespaceJs', (err, res) =>
        ...
        expect(res.body).toContain(@bundle.js.files[0].url)
        expect(res.body).toContain(@bundle.js.files[1].url)
        jasmine.asyncSpecDone()
      jasmine.asyncSpecWait()

    it 'should only render the custom_namespace.js bundle', ->
      request.get 'http://localhost:1338/custom_namespaceJs', (err, res) =>
        expect(@bundle.js.files[1].namespace).toEqual('custom_namespace')
        expect(res.body).toContain(@bundle.js.files[1].url)

        expect(res.body).toNotContain(@bundle.js.files[0].url)
        jasmine.asyncSpecDone()
      jasmine.asyncSpecWait()

  describe 'renderStyles', ->
    it 'should render the global.css bundle', ->
    ...
    it 'should render the global.css and the print_namespace.css bundle', ->
      request.get 'http://localhost:1338/globalJs/printNamespaceCss', (err, res) =>
        expect(@bundle.css.files[0].namespace).toEqual('global')
        expect(@bundle.css.files[1].namespace).toEqual('print_namespace')
        expect(res.body).toContain(@bundle.css.files[0].url)
        expect(res.body).toContain(@bundle.css.files[1].url)
        jasmine.asyncSpecDone()
      jasmine.asyncSpecWait()

    it 'should only render the print_namespace.css bundle', ->
      request.get 'http://localhost:1338/print_namespaceCss', (err, res) =>
        expect(@bundle.css.files[1].namespace).toEqual('print_namespace')
        expect(res.body).toContain(@bundle.css.files[1].url)

        expect(res.body).toNotContain(@bundle.css.files[0].url)
        jasmine.asyncSpecDone()
      jasmine.asyncSpecWait()
